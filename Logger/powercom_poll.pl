...
sub parse_powercom_output {
    ...
    my ( $heat_sink_temperature, $panel_1_voltage, $panel_1_current, $working_hours
    ...
    while ( my $line = <INPUT> ) {
        ...
        if ( $line =~ /serial\snumber\s(\d*)/ ) {
            ...
            $serial_number = $1;
	    print "Inverter Serial Number: $serial_number\n";
	...
	print "Reading DateTime   : $reading_datetime\n";
	print "Accumulated Energy : $accumulated_energy\n";
	...
	print "\n";
        ...
        my $insert_sth = $globals->{db}->prepare(
          ...
          . "  , panel_1_current, working_hours, operating_mode, tmp_f_value, pv_1_f_value, gfci_f_value\n"
